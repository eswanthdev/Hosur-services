"""REST API for the Hosur Services frontend, mounted under /api.

Legacy resources use client-generated ids; provider applications and their
approved listings use server-generated ids.
"""

import os
import re
from datetime import date, datetime, timedelta, timezone
from functools import lru_cache
from pathlib import Path
from typing import Annotated, Literal

from fastapi import APIRouter, Depends, HTTPException
from pydantic import AwareDatetime, BaseModel, BeforeValidator, ConfigDict, Field, HttpUrl, StringConstraints, field_validator, model_validator
from pydantic.alias_generators import to_camel

from hosur.store import ApplicationError, Store, normalize_india_mobile

DEFAULT_DB_PATH = Path(__file__).resolve().parent.parent / "data" / "hosur.db"


@lru_cache
def get_store() -> Store:
    return Store(os.getenv("HOSUR_DB_PATH", DEFAULT_DB_PATH))


class CamelModel(BaseModel):
    model_config = ConfigDict(alias_generator=to_camel, populate_by_name=True)

    def dump(self) -> dict:
        return self.model_dump(by_alias=True)


NonEmpty = Annotated[str, StringConstraints(strip_whitespace=True, min_length=1)]


class ServiceIn(CamelModel):
    id: NonEmpty
    name: NonEmpty
    tamil: NonEmpty
    category: NonEmpty


class ProviderIn(CamelModel):
    id: NonEmpty
    service_id: NonEmpty
    name: NonEmpty
    phone: NonEmpty
    rating: float | None = Field(default=None, ge=1, le=5)
    experience: NonEmpty
    area: str | None = None


class ProviderAreaIn(CamelModel):
    area: str | None = None


class ProviderApplicationIn(CamelModel):
    model_config = ConfigDict(extra="forbid", strict=True)

    name: Annotated[str, StringConstraints(strip_whitespace=True, min_length=1, max_length=120)]
    phone: Annotated[str, StringConstraints(min_length=10, max_length=32)]
    service_ids: list[Annotated[str, StringConstraints(strip_whitespace=True, min_length=1, max_length=100)]] = Field(min_length=1, max_length=10)
    area: Annotated[str, StringConstraints(strip_whitespace=True, min_length=1, max_length=200)]
    experience: Annotated[str, StringConstraints(strip_whitespace=True, max_length=500)] = ""
    language: Literal["en", "ta", "te", "kn"]
    consent: Literal[True]

    @field_validator("phone")
    @classmethod
    def normalize_phone(cls, value):
        return normalize_india_mobile(value)

    @field_validator("service_ids")
    @classmethod
    def unique_services(cls, value):
        if len(set(value)) != len(value):
            raise ValueError("Selected service IDs must be unique.")
        return value

    @field_validator("consent", mode="before")
    @classmethod
    def require_consent(cls, value):
        if value is not True:
            raise ValueError("Consent must be true.")
        return value


class ReviewIn(CamelModel):
    id: NonEmpty
    provider_id: NonEmpty
    user_name: NonEmpty
    rating: float = Field(ge=1, le=5)
    comment: NonEmpty
    created_at: str | None = None


class NewsItemIn(CamelModel):
    id: NonEmpty
    badge: NonEmpty
    area: NonEmpty
    title: NonEmpty
    summary: NonEmpty


class FeedPostIn(CamelModel):
    id: NonEmpty
    author: NonEmpty
    handle: NonEmpty
    location: NonEmpty
    title: NonEmpty
    caption: NonEmpty
    accent: NonEmpty
    tag: NonEmpty
    created_at: str | None = None


class LikeIn(CamelModel):
    user_id: NonEmpty


class AskedForIn(CamelModel):
    query: NonEmpty


INDIA_TIMEZONE = timezone(timedelta(hours=5, minutes=30))


class VerifiedUpdateIn(CamelModel):
    id: NonEmpty
    source_url: HttpUrl
    verified_at: date

    @field_validator("verified_at", mode="before")
    @classmethod
    def validate_verification_date(cls, value):
        if not isinstance(value, str) or not re.fullmatch(r"[0-9]{4}-[0-9]{2}-[0-9]{2}", value):
            raise ValueError("verifiedAt must be a YYYY-MM-DD date.")
        return value

    @field_validator("verified_at")
    @classmethod
    def reject_future_verification(cls, value):
        if value > datetime.now(INDIA_TIMEZONE).date():
            raise ValueError("verifiedAt cannot be in the future in India.")
        return value

    def dump(self) -> dict:
        return self.model_dump(mode="json", by_alias=True)


def validate_iso_timestamp(value):
    if not isinstance(value, str):
        raise ValueError("Timestamp must be an ISO string with a timezone.")
    datetime.fromisoformat(value.replace("Z", "+00:00"))
    return value


IsoTimestamp = Annotated[AwareDatetime, BeforeValidator(validate_iso_timestamp)]


class PowerShutdownIn(VerifiedUpdateIn):
    title: NonEmpty
    areas: list[NonEmpty] = Field(min_length=1)
    starts_at: IsoTimestamp
    ends_at: IsoTimestamp
    reason: NonEmpty

    @model_validator(mode="after")
    def validate_time_range(self):
        if self.ends_at <= self.starts_at:
            raise ValueError("endsAt must be after startsAt.")
        return self


class ChargingStationIn(VerifiedUpdateIn):
    name: NonEmpty
    area: NonEmpty
    address: NonEmpty
    connectors: NonEmpty
    hours: NonEmpty


class CivicAlertIn(VerifiedUpdateIn):
    title: NonEmpty
    category: Literal["water", "traffic", "waste"]
    areas: list[NonEmpty]
    route: Annotated[str, StringConstraints(strip_whitespace=True)] = ""
    message: NonEmpty
    starts_at: IsoTimestamp
    expires_at: IsoTimestamp

    @model_validator(mode="after")
    def validate_alert(self):
        if self.category == "traffic":
            if not self.route:
                raise ValueError("Traffic alerts require a route.")
        elif not self.areas:
            raise ValueError("Water and waste alerts require at least one area.")
        if self.expires_at <= self.starts_at:
            raise ValueError("expiresAt must be after startsAt.")
        return self


router = APIRouter(prefix="/api")


def _created(result, conflict: str):
    if result is None:
        raise HTTPException(status_code=409, detail=conflict)
    return result


@router.get("/state")
def get_state(store: Store = Depends(get_store)):
    return store.get_state()


@router.get("/updates")
def get_updates(store: Store = Depends(get_store)):
    return store.get_updates()


@router.post("/shutdowns", status_code=201)
def add_shutdown(shutdown: PowerShutdownIn, store: Store = Depends(get_store)):
    return _created(store.add_shutdown(shutdown.dump()), "A shutdown with this id already exists.")


@router.put("/shutdowns/{shutdown_id}")
def update_shutdown(shutdown_id: str, shutdown: PowerShutdownIn, store: Store = Depends(get_store)):
    if shutdown.id != shutdown_id:
        raise HTTPException(status_code=422, detail="Body id must match the shutdown id.")
    result = store.update_shutdown(shutdown.dump())
    if result is None:
        raise HTTPException(status_code=404, detail="Shutdown not found.")
    return result


@router.delete("/shutdowns/{shutdown_id}", status_code=204)
def delete_shutdown(shutdown_id: str, store: Store = Depends(get_store)):
    if not store.delete_shutdown(shutdown_id):
        raise HTTPException(status_code=404, detail="Shutdown not found.")


@router.post("/charging-stations", status_code=201)
def add_charging_station(station: ChargingStationIn, store: Store = Depends(get_store)):
    return _created(store.add_charging_station(station.dump()), "A charging station with this id already exists.")


@router.put("/charging-stations/{station_id}")
def update_charging_station(station_id: str, station: ChargingStationIn, store: Store = Depends(get_store)):
    if station.id != station_id:
        raise HTTPException(status_code=422, detail="Body id must match the charging station id.")
    result = store.update_charging_station(station.dump())
    if result is None:
        raise HTTPException(status_code=404, detail="Charging station not found.")
    return result


@router.delete("/charging-stations/{station_id}", status_code=204)
def delete_charging_station(station_id: str, store: Store = Depends(get_store)):
    if not store.delete_charging_station(station_id):
        raise HTTPException(status_code=404, detail="Charging station not found.")


@router.post("/civic-alerts", status_code=201)
def add_civic_alert(alert: CivicAlertIn, store: Store = Depends(get_store)):
    return _created(store.add_civic_alert(alert.dump()), "A civic alert with this id already exists.")


@router.put("/civic-alerts/{alert_id}")
def update_civic_alert(alert_id: str, alert: CivicAlertIn, store: Store = Depends(get_store)):
    if alert.id != alert_id:
        raise HTTPException(status_code=422, detail="Body id must match the civic alert id.")
    result = store.update_civic_alert(alert.dump())
    if result is None:
        raise HTTPException(status_code=404, detail="Civic alert not found.")
    return result


@router.delete("/civic-alerts/{alert_id}", status_code=204)
def delete_civic_alert(alert_id: str, store: Store = Depends(get_store)):
    if not store.delete_civic_alert(alert_id):
        raise HTTPException(status_code=404, detail="Civic alert not found.")


@router.post("/services", status_code=201)
def add_service(service: ServiceIn, store: Store = Depends(get_store)):
    return _created(store.add_service(service.dump()), "A service with this id already exists.")


@router.post("/providers", status_code=201)
def add_provider(provider: ProviderIn, store: Store = Depends(get_store)):
    return _created(store.add_provider(provider.dump()), "Unknown service, or this provider is already listed for it.")


def _application_result(operation, *args):
    try:
        return operation(*args)
    except ApplicationError as exc:
        raise HTTPException(status_code=exc.status_code, detail=exc.detail) from exc


@router.post("/provider-applications", status_code=201)
def add_provider_application(application: ProviderApplicationIn, store: Store = Depends(get_store)):
    return _application_result(store.add_provider_application, application.dump())


# LOCAL prototype only: protect queue and review routes with Cognito admin auth before deployment.
@router.get("/provider-applications")
def get_provider_applications(store: Store = Depends(get_store)):
    return store.get_provider_applications()


@router.post("/provider-applications/{application_id}/approve")
def approve_provider_application(application_id: str, store: Store = Depends(get_store)):
    return _application_result(store.approve_provider_application, application_id)


@router.post("/provider-applications/{application_id}/reject")
def reject_provider_application(application_id: str, store: Store = Depends(get_store)):
    return _application_result(store.reject_provider_application, application_id)


@router.patch("/providers/{provider_id}")
def update_provider(provider_id: str, update: ProviderAreaIn, store: Store = Depends(get_store)):
    provider = store.update_provider_area(provider_id, update.area)
    if provider is None:
        raise HTTPException(status_code=404, detail="Provider not found.")
    return provider


@router.delete("/providers/{provider_id}", status_code=204)
def delete_provider(provider_id: str, store: Store = Depends(get_store)):
    if not store.delete_provider(provider_id):
        raise HTTPException(status_code=404, detail="Provider not found.")


@router.post("/reviews", status_code=201)
def add_review(review: ReviewIn, store: Store = Depends(get_store)):
    return _created(store.add_review(review.dump()), "Unknown provider, or this review already exists.")


@router.post("/news", status_code=201)
def add_news_item(item: NewsItemIn, store: Store = Depends(get_store)):
    return _created(store.add_news_item(item.dump()), "A news item with this id already exists.")


@router.post("/feed", status_code=201)
def add_feed_post(post: FeedPostIn, store: Store = Depends(get_store)):
    return _created(store.add_feed_post(post.dump()), "A post with this id already exists.")


@router.post("/feed/{post_id}/like")
def like_post(post_id: str, like: LikeIn, store: Store = Depends(get_store)):
    post = store.like_post(post_id, like.user_id)
    if post is None:
        raise HTTPException(status_code=404, detail="Post not found.")
    return post


@router.post("/asked-for", status_code=201)
def add_asked_for(entry: AskedForIn, store: Store = Depends(get_store)):
    return store.add_asked_for(entry.query.strip())
